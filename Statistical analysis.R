...
process_csv_file <- function(file_path) {
  ...
  # Steel-Dwass
  run_steel_dwass_nsm3 <- function(data, group_col, value_col) {
      ...
      for (j in (i + 1):length(group_levels)) {
        ...
        if (length(x1) >= 2 && length(x2) >= 2) {
          test_result <- tryCatch({
            p <- NSM3::pSDCFlig(x = list(x1, x2), method = "Steel-Dwass")$p.val
            data.frame(
              group1 = g1, group2 = g2, p_adj = p,
              p_label = case_when(
                p < 0.001 ~ "(***)",
                p < 0.01 ~ "(**)",
                p < 0.05 ~ "(*)",
  ...
  steel_df <- run_steel_dwass_nsm3(df_long, "organ", "value")
